<x-app-layout>
  <x-slot name="header">
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
          <a href="{{ route('dashboard.product.index') }}">Product</a>{!! __(' &raquo; Edit') !!}
      </h2>
  </x-slot>
              ...
              <form class="w-full" action="{{ route('dashboard.product.update', $item->id) }}" method="post" enctype="multipart/form-data">
                  ...
                  <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <input value="{{ old('photo_url') ?? $item->photo_url }}" name="file" type="file" accept="image/*" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" placeholder="Photos">
                    </div>
                  </div>
                  {{-- End Foto --}}
                  <div class="flex flex-wrap -mx-3 mb-6">
